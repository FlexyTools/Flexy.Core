...
namespace Flexy.Core.Binding
{
	public abstract class CallBinder : MonoBehaviour
	{
		...
		private record		OneParamBinder<TParam>		( TParam Param, Action<TParam> M )				{ public void SetValue	( ) => M(Param);			}
		private record		ZeroParamBinderVoid			( Func<UniTaskVoid> M )							{ public void SetValue	( ) => M();					}
		private record		OneParamBinderVoid<TParam>	( TParam Param, Func<TParam, UniTaskVoid> M )	{ public void SetValue	( ) => M(Param).Forget();	}
	}
}