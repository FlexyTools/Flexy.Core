...
namespace Flexy.Core.GameContexts
{
	...
	public class GameContext : MonoBehaviour
	{
		...
		public			void			LinkScene				( Scene scene )		
		{
			...
			if (!scene.IsValid())
			{
				Debug.LogWarning("Trying to link invalid scene", this);
				return;
			}
		...
		protected		void			Awake					( )		
		{
			...
			if (_global == this)
				Debug.Log( $"[Frame:{Time.frameCount}] \t global", this );
			else
				Debug.Log( $"[Frame:{Time.frameCount}] \t parent:{_parent}", this );

			switch (LinkTo)
			{
				case ELinkCtxTo.AllScenes:
				{
					Debug.Log( $"[Frame:{Time.frameCount}] Link Scenes: All", this );
				...
				{
					Debug.Log( $"[Frame:{Time.frameCount}] Link Scenes: One", this );
					LinkScene( gameObject.scene );
					break;
				}
				default:
				{
					Debug.Log( $"[Frame:{Time.frameCount}] Link Scenes: None", this );
					break;
				}
			}
		...
		protected		void			OnDestroy				( )		
		{
			...
			Debug.Log( $"[Frame:{Time.frameCount}] {name} \t parent:{_parent}", this );
		...
		public			void			SetService<T>			( T service, Boolean replace = false )	where T : class	
		{
			...
			{
				Debug.Log(GetDisplayServiceName(typeActual), this);
				try 
				{
			...
			foreach (var serviceType in serviceTypes.InterfaceType)
			{
				...
				{
					Debug.LogError	( $"{GetDisplayServiceName(serviceType)} is not implemented by {GetDisplayServiceName(typeActual)}" );
					continue;
				}

				Debug.Log	( $"{GetDisplayServiceName(serviceType)} => {GetDisplayServiceName(typeActual)}", this );
				try 
				{
		...
		private static	GameContext		CreateGlobalContext		( )												
		{
			...
			Debug.Log($"[Frame:{Time.frameCount}] {ctx.name}", ctx);
		...
		{
			Debug.Log( $"[Frame:{Time.frameCount}] {scene.name}" );
		...
		private static 	void			LinkCreatedScene		( Scene oldScene, Scene newScene )				
		{
			...
			Debug.Log( $"[Frame:{Time.frameCount}] {ctx.name} -> {newScene.name}" );
			ctx.LinkScene(newScene);
		}
		private static 	void			LinkSideLoadedScene		( Scene newScene, LoadSceneMode loadSceneMode )	
		{
			if (!IsGlobalAlive || _sceneToCtxLinks.ContainsKey(newScene)) 
				return;
			...
			{
				Debug.LogWarning( $"[Frame:{Time.frameCount}] active scene {scene.name} not connected to Ctx!" );
			}
		...
		public override	String			ToString	( ) => $"{(this?name:"dead")} {GetType().Name}";
